...
//编码能力提升计划，二分法，https://renjie.blog.csdn.net/article/details/133141372
    ...
    private static Integer isPass(int mid){
        ...
        for (int i = 0; i < firstHang.length; i++) {
            ...
            if(sum - kandaan2>mid){//减去最耗时题，加上当前元素时的sum - kandaan>mid。
                maxTime = Math.max(maxTime, sum-firstHang[i]-kandaan);//记录跳过后的最大耗时，sum-firstHang[i]-kandaan，sum减去(不含今天的最大耗时)，减去当前元素
                count++;//指向下一天
                sum=0;//重置，为了计算第二天
