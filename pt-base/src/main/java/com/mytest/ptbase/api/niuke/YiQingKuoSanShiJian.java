...
//        String[] firstHang=in.nextLine().split(" ");
    ...
    private static void bfsFind(LinkedList<int[]> deque) {
        LinkedList<int[]> dequeNext = new LinkedList<>();
        while (deque.size() != 0) {
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (0 <= x && x < n && 0 <= y && y < n && matrix[x][y] == 0) {
                    matrix[x][y] = 1;
                    dequeNext.addLast(new int[]{x, y});
                }
            }
            if (deque.isEmpty()&&!dequeNext.isEmpty()) {
                res++;
                deque.addAll(dequeNext);
                dequeNext.removeAll(dequeNext);
            }
