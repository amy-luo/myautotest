...
//        String[] firstHang=in.nextLine().split(" ");
    ...
    private static void calDiguiHuisu(int row,int count){
        if(row==n){list.add(count);return;}
        for (int i = row; i < n; i++) {
            if(i==0||(i>0&&matrix[i][0]>=matrix[i-1][1]+15)){
                calDiguiHuisu(i+1,count+1);
            }else{calDiguiHuisu(i+1,count);}//else一定要加，不加当前元素时count不会加1，这个时候也要回溯，不然无法触发row==n的结束条件。
        }
    }
}
