...
public class JiaMiSuanFa {
    ...
    public static void main(String[] args) {
        ...
        while (in.hasNextLine()) {
            ...
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < m; j++) {
                    StringBuilder stb;stb = new StringBuilder();
                    if(matrix[i][j]==nums[0]&&isFind(i,j,1,stb)){
                        flag=true;
                        break;
                    }
                }
    ...
    private static boolean isFind(int i,int j,int count,StringBuilder stb){
        stb.append(i+" "+j+" ");
        if(count==n){
            result= stb.toString();
            return true;
